...
int main(int argc, char **argv)
{
    ...
    auto const node =
        rclcpp::Node::make_shared("panda_safe_picker");
    ...
    std::future<void> async_executor =
        std::async(
            std::launch::async,
            [&executor]()
            {
    ...
    // =========================================================
    auto attach_client =
        node->create_client<linkattacher_msgs::srv::AttachLink>(
            "/ATTACHLINK");

    auto detach_client =
        node->create_client<linkattacher_msgs::srv::DetachLink>(
            "/DETACHLINK");
    ...
    cube_color.color.r = 1.0;
    cube_color.color.g = 0.0;
    cube_color.color.b = 0.0;
    cube_color.color.a = 1.0;

    moveit_msgs::msg::ObjectColor table_color;

    table_color.id = "table_surface";

    table_color.color.r = 0.58;
    table_color.color.g = 0.29;
    ...
    std::vector<double> gripper_close = {
        0.00,
        0.00
    };
    ...
    approach_pose.position.x = 0.40;
    approach_pose.position.y = 0.0;
    approach_pose.position.z = 0.20;
    ...
    if (arm_group.plan(plan) !=
        moveit::core::MoveItErrorCode::SUCCESS)
    {
        RCLCPP_ERROR(
            node->get_logger(),
            "Failed to plan Approach pose!");
    ...
    // =========================================================
    // 2. STAGE: OPEN FINGERS
    // IMPORTANT:
    // This happens BEFORE Cartesian lowering.
    // =========================================================
    RCLCPP_INFO(
        node->get_logger(),
        "STAGE: Opening Fingers Before Lowering");

    hand_group.setJointValueTarget(gripper_open);

    if (hand_group.plan(plan) !=
        moveit::core::MoveItErrorCode::SUCCESS)
    {
        RCLCPP_ERROR(
            node->get_logger(),
    ...
    // =========================================================
    // 3. STAGE: CARTESIAN LOWERING
    // =========================================================
    geometry_msgs::msg::Pose pick_pose =
        approach_pose;

    pick_pose.position.z = 0.130;
    ...
    double fraction =
        arm_group.computeCartesianPath(
            cartesian_waypoints,
            eef_step,
            jump_threshold,
            trajectory);
    ...
    RCLCPP_INFO(
        node->get_logger(),
        "Visualizing Cartesian Lowering path "
        "(%.2f%% achieved)",
        fraction * 100.0);
    ...
    if (fraction < 0.9)
    {
        RCLCPP_ERROR(
            node->get_logger(),
            "Cartesian path failed to compute fully!");
    ...
    psi.applyAttachedCollisionObject(
        allow_touch);
    ...
    RCLCPP_INFO(
        node->get_logger(),
        "STAGE: Grasp - Sending close command");

    hand_group.setJointValueTarget(
        gripper_close);
        ...
        moveit::core::MoveItErrorCode::SUCCESS)
    {
        ...
        auto request =
            std::make_shared<
                linkattacher_msgs::srv::AttachLink::Request>();
        ...
        if (!attach_client->wait_for_service(
        ...
        {
            auto result =
                attach_client->async_send_request(
                    request);
        }
    }
    else
    ...
    else
    {
        RCLCPP_ERROR(
            node->get_logger(),
            "Cartesian lift failed!");
    }

    // =========================================================
    // 7. STAGE: ABOVE PLACE POSITION
    // FREE-SPACE TRANSIT
    // =========================================================
    geometry_msgs::msg::Pose above_place_pose;

    above_place_pose.orientation =
        downward_orientation;

    above_place_pose.position.x = 0.25;
    above_place_pose.position.y = 0.30;
    above_place_pose.position.z = 0.20;

    RCLCPP_INFO(
        node->get_logger(),
        "STAGE: Move to above place pose "
        "(free-space)");
    ...
    else
    {
        RCLCPP_ERROR(
            node->get_logger(),
            "Failed to plan above-place movement!");
    }

    // =========================================================
    // 8. STAGE: PLACE POSITION
    // =========================================================
    ...
    std::vector<geometry_msgs::msg::Pose>
        final_drop_waypoints = {
            place_pose
        };

    RCLCPP_INFO(
        node->get_logger(),
        "STAGE: Cartesian Place");
    ...
    else
    {
        RCLCPP_ERROR(
            node->get_logger(),
            "Cartesian place failed!");
    }

    // =========================================================
    // 9. STAGE: OPEN GRIPPER AND DETACH
    // =========================================================
    RCLCPP_INFO(
        node->get_logger(),
        "STAGE: Opening Gripper and Detaching Cube");
        ...
        moveit::core::MoveItErrorCode::SUCCESS)
    {
        ...
        // =====================================================
        // DETACH LINK IN GAZEBO
        // =====================================================
        auto detach_request =
            std::make_shared<
                linkattacher_msgs::srv::DetachLink::Request>();
        ...
        if (!detach_client->wait_for_service(
                std::chrono::seconds(10)))
        {
            RCLCPP_ERROR(
                node->get_logger(),
                "Service /DETACHLINK still not available!");
        }
        else
        {
            auto result =
                detach_client->async_send_request(
                    detach_request);
            ...
            RCLCPP_INFO(
                node->get_logger(),
                "Gazebo: Cube successfully detached.");
        }
    }
    else
    {
        RCLCPP_ERROR(
            node->get_logger(),
            "Failed to plan gripper opening at place!");
    }

    // =========================================================
    // 10. FINAL STAGE: CLEARING MOVE
    // =========================================================
